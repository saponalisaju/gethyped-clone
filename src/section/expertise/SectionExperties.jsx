...
function ExpertiseSection() {
  ...
  useEffect(() => {
      ...
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            const index = panelsRef.current.findIndex(
              (el) => el === entry.target,
            );
    ...
    return () => {
      panelsRef.current.forEach((el) => el && observer.unobserve(el));
    };
  }, []);

  return (
    <section ref={sectionRef} className="expertise" id="expertises">
          ...
          <div className="expertise__detail">
            {expertises.map((e, i) => (
              <div
                ...
                className="expertise__panel-wrapper"
              >
                <div
                  className={`expertise__panel${active === i ? " expertise__panel--active" : ""}`}
                  aria-hidden={active !== i}
                >
                  <div className="expertise__panel-video w-25">
                    <video
                      src={e.video}
                      muted
                      loop
